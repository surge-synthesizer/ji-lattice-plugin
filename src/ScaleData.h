...
// Here it is as ratios, fractions, and co-ordinates:
...
static constexpr frac_t pyth12fractions[12] = {
...
struct comma_t
{
    ...
    constexpr comma_t(CommaNames n, frac_t f, coord_t c = {-4, 1}) : name(n), fraction(f), coord(c)
    {
    ...
    double getRatio(int degree) const
    {
        if (isDegreeMajor[degree])
            return fraction.first / fraction.second;

        return fraction.second / fraction.first;
    }
    ...
    // used for syntonic mode
    void enableDiesis(int degree)
    {
        if (name != syntonic || !(degree == 6 || degree == 11 || degree == 4))
        {
            return;
        }
        // syntonic comma and diesis combine into a diaschisma (2048/2025)
        fraction = std::make_pair(2048, 2025);
        coord.second = -2;
    }
    void disableDiesis(int degree)
    {
        if (name != syntonic || !(degree == 6 || degree == 11 || degree == 4))
        {
            return;
...
static constexpr comma_t commas[16] = {comma_t(none, {1, 1}, {0, 0}),
...
static constexpr CommaNames defaultCommas[12] = {none,     syntonic, none,     syntonic,
                                                 syntonic, none,     syntonic, none,
                                                 syntonic, syntonic, syntonic, syntonic};

struct ScaleData
{
    ScaleData(std::string n, const int *v = nullptr) : ScaleName(n)
    {
        if (v)
        {
    ...
    void setDegree(int d, CommaNames c)
    {
        CC[d] = commas[c];
    ...
    void setName(std::string n) { ScaleName = n; }

    void resetToDefault()
    {
        for (int d = 0; d < 12; ++d)
        {
            CC[d] = commas[defaultCommas[d]];
            CT[d] = pyth12ratios[d] * CC[d].getRatio(d);
            CO[d] = pyth12coords[d];
